...
def toggle_remaining_due_counts():
    ...
    if config is not None:
        if "shortcut" in config and config['shortcut']:
            ...
            if key_seq.matches(Qt.Key.Key_unknown) != QKeySequence.SequenceMatch.NoMatch:
                showInfo(f"WARNING: The configured keyboard shortcut for the Toggle Remaining Card Counts add-on is invalid! Please double-check the configuration under Tools > Add-Ons.")

    action = QAction("Toggle Remaining Card Counts", mw)
    qconnect(action.triggered, toggle_remaining_due_counts)
